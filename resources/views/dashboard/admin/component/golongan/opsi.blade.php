        ...
        <div class="modal-content">
            <div class="modal-header">
                <div class="d-flex justify-contemt-center align-items-center gap-3">
                    <h4 class="modal-title btn-secondary fs-5" id="hapusLabel">UPDATE</h4>
                    <span class="btn btn-primary ml-2">{{ $model->g_nama }}</span>
                </div>
                ...
                <div class="container">
                    <div class="row">
                        ...
                        <x-form-dashboard title="" method="PUT"
                            url="{{ route('admin.dashboard.golongan.update', ['golongan' => $model->id]) }}"
                            :listForm="$list" />
                    </div>
                </div>
            </div>
                    ...
                    <div class="row">
                        <p class="text-center">
                            Apakah yakin ingin menghapus <strong
                                class="text-capitalize">"{{ $model->g_nama }}"</strong>?
                        </p>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <form method="post" action="{{ route('admin.dashboard.golongan.destroy', $model->id) }}">
                    @csrf
                    @method('delete')
