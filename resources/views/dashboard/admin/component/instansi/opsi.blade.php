        ...
        <div class="modal-content">
            <div class="modal-header">
                <div class="d-flex justify-contemt-center align-items-center gap-3">
                    ...
                    <span class="btn btn-primary ml-2">{{ $model->i_nama }}</span>
                </div>
                ...
                <div class="container">
                    <div class="row">
                        ...
                        <x-form-dashboard title="" method="PUT"
                            url="{{ route('admin.dashboard.instansi.update', ['instansi' => $model->id]) }}"
                            :listForm="$list" />
                    </div>
                </div>
            </div>
                    ...
                    <div class="row">
                        <p class="text-center">
                            Apakah yakin ingin menghapus <strong
                                class="text-capitalize">"{{ $model->i_nama }}"</strong>?
                        </p>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <form method="post" action="{{ route('admin.dashboard.instansi.destroy', $model->id) }}">
                    @csrf
                    @method('delete')
